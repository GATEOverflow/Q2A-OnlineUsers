...
class show_online_user_count_widget {
	...
	function output_widget($region, $place, $themeobject, $template, $request, $qa_content)
	{
		...
		$total_online_html='<span class="show-online-user-count-data">'.($online_member_count['COUNT(*)']).'</span>';
		...
		if(($online_member_count['COUNT(*)']) > 1)
			$tempStr=str_replace('^1',$online_member_html,qa_lang_html('show_online_user_count_lang/online_guest_member'));
		else
			$tempStr=str_replace('^1',$online_member_html,qa_lang_html('show_online_user_count_lang/one_online_guest_member'));
	...
	//	$themeobject->output(str_replace('^',$total_online_html,qa_lang_html('show_online_user_count_lang/total_online_users')));
...
//		$themeobject->output('</div>');
		...
		{
			$query="SELECT distinct ^users.handle FROM ^users,^online_user WHERE ^users.userid=^online_user.user_id AND ^online_user.user_id>0";
			$resultDb=qa_db_query_sub($query);
